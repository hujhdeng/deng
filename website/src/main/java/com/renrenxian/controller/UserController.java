...
import com.io.rong.ApiHttpClient;
import com.io.rong.FormatType;
import com.io.rong.SdkHttpResult;
...
public class UserController {
	...
	public static String[] keys = new String[] { "uName", "company", "kpno",
			"address", "sex", "old", "area", "keyword", "industy", "business",
			"year", "cont", "alert", "locat", "alert2" };
	...
	public Object updatepwd(HttpServletRequest req,
			...
			@RequestParam(value = "lng", required = false) String lng,
			@RequestParam(value = "lat", required = false) String lat) {
		logger.info("phone:{}, password:{},yzm:{}, lng:{},lat:{}",
				new String[] { phone, password, yzm, lng, lat });
		String callback = req.getParameter("callback");
		try {
			// TODO 密码长度 交互安全问题
			// 检查参数
	...
	public Map<String, Object> userinfo(HttpServletRequest httpServletRequest,
		...
		logger.info("uid:{}, lng:{},lat:{}", new String[] { uid, lng, lat });
		try {
	...
	@ResponseBody
	public Map<String, Object> userinfoByMy(
			HttpServletRequest httpServletRequest,
			@RequestParam(value = "uid", required = true) String uid,
			@RequestParam(value = "myid", required = true) String myid) {

		logger.info("uid:{}, myid:{}", new String[] { uid, myid });
		try {
			int id = StringUtil.parseInt(uid, 0);
			if (id == 0) {
				return MapResult.initMap(1001, "用户id错误！");
	...
	public Map<String, Object> updateUser(
		...
		try {
			...
			Map<String, Object> map = userService.updateKV(id, key, value, lng,
					lat);
			return map;
		} catch (Exception ex) {
			logger.error("", ex);
	...
	@ResponseBody
	public Map<String, Object> replenish(HttpServletRequest request, User user) {
	...
	@ResponseBody
	public Object addwxFollow(HttpServletRequest req,
	...
	public Map<String, Object> followbothlist(HttpServletRequest request,
			@RequestParam(value = "uid", required = true) String uid) {
		logger.info("followbothlist  uid:{}", new Object[] { uid });
		int id = StringUtil.parseInt(uid, 0);
		if (id == 0) {
			return MapResult.initMap(1001, "用户id错误！");
		}
		...
		try {
			...
			if (list == null) {
				...
				if (list.size() == 0) { // 没有相互关注的人
					...
					Map<String, Object> map = MapResult.initMap();
					/**
					 * JSONArray array = new JSONArray(); JSONObject object =
					 * null; for (User user : list) { object = new JSONObject();
					 * object.put("uid", user.getId()); object.put("avatar",
					 * user.getAvatar()); array.add(object); } map.put("data",
					 * array);
					 **/
					map.put("data", list);
					logger.info("return map:{}", map);
	...
	public Map<String, Object> search(
		...
		logger.info(
				"phone:{}, uName:{}, indstry:{}, area:{}, business:{}, page:{}, pagesize:{}",
				new Object[] { phone, uName, industy, area, business, page,
						pagesize });
		...
		try {
			...
			for (Entry<String, String> e : set) {
				where = new WhereWrapper(e.getKey(), WhereWrapper.SYMBOL_LIKE,
						e.getValue());
				whereList.add(where);
			}
			// order by kpno desc
			...
			sortList.add(sort);
			//
			Map<String, Object> map = userService.search(id, whereList,
					sortList, page, pagesize);
			return map;
		} catch (Exception ex) {
			logger.error("", ex);
			return MapResult.failMap();
	...
	public Map<String, Object> friends(
		...
		try {
			Map<String, Object> map = userService.findByPhones(id, phonelist,
					page, pagesize);
			logger.info("return map:{}", map);
			return map;
		} catch (Exception ex) {
	...
	// 附近的人
	/**
	 * 小经度 minlng 大经度 malling 小维度 minlat 大维度 maxlat page pagesize (默认20)
	 **/
	@RequestMapping(value = "/near")
	@ResponseBody
	public Map<String, Object> near(HttpServletRequest request,
			@RequestParam(value = "uid", required = true) String uid,
			...
			@RequestParam(value = "page", required = false) Integer page,
			@RequestParam(value = "pagesize", required = false) Integer pagesize) {
		logger.info("uid:{}, lng:{}, lat:{}, range:{}, pageno:{}, pagesize:{}",
				new Object[] { uid, lng, lat, range, page, pagesize });
		...
		try {
			// Map<String, Object> map = userService.near(id, minlngd, maxlngd ,
			// minlatd, maxlatd, starttime, page, pagesize);
			Map<String, Object> map = userService.near(id, latd, lngd, ranged,
	...
	@ResponseBody
	public Map<String, Object> kwSearch(HttpServletRequest request,
			@RequestParam(value = "uid", required = true) String uid,
		...
		logger.info(
				"kwSearch-->params uid:{}, keyword:{}, page:{}, pagesize:{}",
				new Object[] { uid, keyword, page, pagesize });
		...
		try {
			Map<String, Object> map = userService.kwsearch(id, keyword, page,
					pagesize);
			logger.info("return map:{}", map);
			return map;
		} catch (Exception ex) {
	...
	@ResponseBody
	public Map<String, Object> wxuser(HttpServletRequest request,
			@RequestParam(value = "uid", required = true) String uid) {
		int id = StringUtil.parseInt(uid, 0);
		if (id == 0) {
			return MapResult.initMap(10001, "用户id错误！");
	...
	// 融云接口
	@RequestMapping(value = "/regcloudtoken")
	@ResponseBody
	public Map<String, Object> regcloudtoken(HttpServletRequest request,
			@RequestParam(value = "uid", required = true) String uid)
			throws Exception {
		
		String key = "82hegw5uh7dox";
		String secret = "iWSZCRIbJBq";
		...
		toIds.add("id2");
		toIds.add("id3");
		// SdkHttpResult result2 = ApiHttpClient.publishMessage(key, secret,
		// "fromUserId", toIds,
		// new TxtMessage("txtMessagehaha"), FormatType.json);

		Map<String, Object> map = MapResult.initMap();
		map.put("gettoken", result1);
		// map.put("result", result2.toString());
		return map;
	}
}

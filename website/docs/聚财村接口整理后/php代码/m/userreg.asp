...
<%
'response.Write(session("tracker_l"))
...
u_table="lmuser"
rs.open "select * from "&u_table&" where id="&session("tracker_l") ,conn,1,3
If Not rs.EOF Or Not rs.BOF Then
tj_tel=trim(rs("u_tel"))
tj_name=trim(rs("u_name"))
tj_money="0"
u_tracker=session("tracker_l")
end if
rs.close
end if
...
<div style=" width:95%; margin:auto;  clear:both;"><input name="提交" type="button" style=" background-image:url(images/anbk1.jpg);font-weight:bold; color:#FFF; font-size:1em" onClick="chackreg()" value="立即注册" /></div>

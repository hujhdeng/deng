...
//
//        // 2. 设置请求选项, 包括具体的url
//        curl_setopt($ch, CURLOPT_URL, "http://23.180.vrvr.cn/jcc/push?act=push&uid=".$reid."&msg=".$sinfo);
...
$myrow = mysql_fetch_row($rs);
if(empty($myrow)){
$sql="INSERT INTO ".$u_table.' (seid,reid,regtime,lastcontent,u_name,kpno,avatar) VALUES ("'.$seid.'","'.$reid.'","'.$regtime.'","'.$content.'","'.$seu_name.'","'.$sekpno.'","'.$seavatar.'")';	
	}else{
...
$myrow = mysql_fetch_row($rs);
if(empty($myrow)){
$sql="INSERT INTO ".$u_table.' (seid,reid,regtime,lastcontent,u_name,kpno,avatar,hasread) VALUES ("'.$reid.'","'.$seid.'","'.$regtime.'","'.$content.'","'.$reu_name.'","'.$rekpno.'","'.$reavatar.'","0")';	
	}else{
$sql="update $u_table set regtime='".$regtime."',lastcontent='".$content."',u_name='".$ren_name."',kpno='".$rekpno."',avatar='".$reavatar."',hasread='0' where seid=".$reid." and reid=".$seid;
		}
mysql_query($sql);
...
$id=$_POST['id'];
$uid=$_POST['uid'];
$assessnum=$_POST['assessnum'];
$assesstxt=$_POST['assesstxt'];
...
/*//---------------------------百度云推送
	...
	$sinfo='{"type":"1","message":{"uid":"'.$seid.'","uname":"'.$uname.'","avatar":"'.$avatar.'","content":"'.$message.'","sdid":"'.$id.'","sduid":"'.$sduid.'"}}';
        ...
        // 2. 设置请求选项, 包括具体的url
        curl_setopt($ch, CURLOPT_URL, "http://23.180.vrvr.cn/jcc/push?act=push&uid=".$reid."&msg=".$message);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_HEADER, 0);
...
if(strstr($joinlist,'|'.$uid)!=''){
	$arr = array ('apicode'=>10001,'data'=>array ('message'=>urlencode('重复申请！')));
	}else{
...
$industy=$_POST['industy'];
$business=$_POST['business'];
//$dan_count=$_POST['dan_count'];
$year=$_POST['year'];
$cont=$_POST['cont'];
//$followcount=$_POST['followcount'];
//$followmecount=$_POST['followmecount'];
//$followbothcount=$_POST['followbothcount'];
//$follow_list=$_POST['follow_list'];
$lnglat=$_POST['lng'].','.$_POST['lat'];

<?php 
if(isset($_GET['uid'])){$wxuid=$_GET['uid'];}else{$wxuid='';}
if(isset($_GET['sat'])){$sat=$_GET['sat'];}else{$sat='';} 
$mysqli = new mysqli('localhost','root', 'sanrenxing', 'jcc');
mysqli_set_charset ($mysqli,'utf8');
//-----------------------以上为公共数据库连接
$query="select * from user where id=".$wxuid;
...
$query="SELECT COUNT(*) FROM user WHERE id<=$myid";
$result=$mysqli->query($query);
$myrow =$result->fetch_array();

//$rs=mysqli_query("SELECT COUNT(*) FROM user WHERE id<=$myid",$con);	
